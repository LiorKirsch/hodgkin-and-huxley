...
public class Neuron {
	...
	public ArrayList<Double> spikeTimes;
	public ArrayList<NeuronalConnection> inputSynapses;
	public ArrayList<NeuronalConnection> outputSynapses;
	public boolean justFired = false;
	//public ConcurrentLinkedQueue<Double> lastSpikeTime ;
	//public ConcurrentLinkedQueue<Double> noiseSpikeTimes ;
	...
	//	this.noiseSpikeTimes = new ConcurrentLinkedQueue<Double>();
	}
	public Neuron (double n, double m, double h,double V, int indexNumberDebug)
	{
		this(n,m,h,V);
		...
		if (this.inputSynapses != null)
		{
			for (NeuronalConnection currentSynapse : inputSynapses) {
				...
				for (int j = 0; j < currentNeuronSpikeTimes.size(); j++) {
					Isyn += -1  * currentSynapse.synapseStrength * mathStuff.NeuronMath.alphaFunction(currentTime -currentNeuronSpikeTimes.get(j) - currentSynapse.synapseLatency ) * (this.V - currentSynapse.vSyn );
				}
			}
		}
		return Isyn;
...
/*	private double calcIsynaptic(double currentTime)
	{
		...
		if (this.inputSynapses != null)
		{
			...
			{
				Iterator<NeuronalConnection> iteratorSynapse = inputSynapses.iterator();
				NeuronalConnection currentSynapse = iteratorSynapse.next(); 
				Neuron currentNeuron = currentSynapse.neuron ;
				ArrayList<Double> currentNeuronSpikeTimes = currentNeuron.getSpikeTimes();
				for (int j = 0; j < currentNeuronSpikeTimes.size(); j++) {
					Isyn += -1  * gSyn * NeuronMath.alphaFunction(currentTime -currentNeuronSpikeTimes.get(j) - currentSynapse.synapseLatency ) * (this.V - currentSynapse.vSyn );
				}
			}
		}
		return Isyn;
	...
	*/
	 ...
	 */
	private NeuronState calcDotProduct (double n, double m, double h,double V, double Iext,double t,double Isyn)
	{
		double alpha_m,beta_m,minf,tau_m,alpha_n,beta_n ,ninf,tau_n,alpha_h,beta_h,hinf,tau_h,I_L,I_Na,I_K;
	 ...
	 */
	private void rk4(double t,double h,double Iext,double Isyn)
	{
		   double hh = h*0.5;
		   //double th = t + hh;
		    /**
		   ...
		   dydt =  this.calcDotProduct(this.n,this.m,this.h,this.V,Iext, t,Isyn);
		    ...
		    dyt = this.calcDotProduct(tempN,tempM,tempH,tempV,Iext, t,Isyn);
		    //yt = y + hh * dyt;
		    ...
		    dym = this.calcDotProduct(tempN,tempM,tempH,tempV,Iext, t,Isyn);
		    //yt = y + hh * dym;
	...
	*/
	...
	//	double Isyn1 = NeuronMath.calcIsynaptic(t,this);
		double Isyn2 = NeuronMath.calcIsynaptic(this, t, dt);
		//double Isyn3 = calcIsynaptic(t);
		//System.out.println("Isyn = " + Isyn + " ,new Isyn = " + Isyn2);
		this.rk4(t, dt,Iext,Isyn2);
	...
	private void checkForSpike(double oldVdot, double t)
	{
	    if (  (oldVdot > 0)  && (this.VDot < 0) && ( this.V >threshold) )
	    {
	    	spikeTimes.add(t);
	    	this.justFired = true;
	  //  	this.lastSpikeTime.add(t);
	    	for (NeuronalConnection synapse : outputSynapses) {
